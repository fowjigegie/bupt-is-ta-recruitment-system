...
$tempWorkspace = New-TempWorkspace -Prefix "ta-us11-test"
...
try {
    ...
    $targetSourceFile = Join-Path $stagedTestSourceDir "US11SmokeTest.java"

    New-Item -ItemType Directory -Path $stagedTestSourceDir -Force | Out-Null
    Copy-Item -Path (Join-Path $testSourceDir "US11SmokeTest.java") -Destination $targetSourceFile -Force
