...
$tempWorkspace = New-TempWorkspace -Prefix "ta-profile-validator-test"
...
try {
    ...
    & $java -cp "$stagedMainOutputDir;$stagedTestOutputDir" com.bupt.tarecruitment.ApplicantProfileValidatorRuleTest
    if ($LASTEXITCODE -ne 0) {
        throw "ApplicantProfileValidatorRuleTest failed."
    }
} finally {
