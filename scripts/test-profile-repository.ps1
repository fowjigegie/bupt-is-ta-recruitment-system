...
$tempWorkspace = New-TempWorkspace -Prefix "ta-profile-repo-test"
...
try {
    ...
    & $java -cp "$stagedMainOutputDir;$stagedTestOutputDir" com.bupt.tarecruitment.applicant.ApplicantProfileRepositoryRoundTripTest
    if ($LASTEXITCODE -ne 0) {
        throw "ApplicantProfileRepositoryRoundTripTest failed."
    }
} finally {
