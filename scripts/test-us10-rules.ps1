...
$tempWorkspace = New-TempWorkspace -Prefix "ta-us10-rule-test"
...
try {
    ...
    & $java -cp "$stagedMainOutputDir;$stagedTestOutputDir" com.bupt.tarecruitment.recommendation.MissingSkillsFeedbackServiceRuleTest
    if ($LASTEXITCODE -ne 0) {
        throw "MissingSkillsFeedbackServiceRuleTest failed."
    }
} finally {
