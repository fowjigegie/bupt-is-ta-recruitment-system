...
$tempWorkspace = New-TempWorkspace -Prefix "ta-profile-service-rule-test"
...
try {
    ...
    & $java -cp "$stagedMainOutputDir;$stagedTestOutputDir" com.bupt.tarecruitment.applicant.ApplicantProfileServiceRuleTest
    if ($LASTEXITCODE -ne 0) {
        throw "ApplicantProfileServiceRuleTest failed."
    }
} finally {
